  ...
  useEffect(() => {
    ...
    filtered.forEach(ticket => {
      ...
      const icon = L.divIcon({
        className: '',
        html: `<div style="width:14px;height:14px;border-radius:50%;background:${color};border:2px solid #0d1117;box-shadow:0 0 6px ${color}80;"></div>`,
        iconSize: [14, 14],
        iconAnchor: [7, 7],
      });
      ...
      const marker = L.marker([coord.lat + (Math.random() - 0.5) * 0.01, coord.lng + (Math.random() - 0.5) * 0.01], { icon })
        .addTo(map)
        .bindPopup(`
          <div style="font-family:Space Grotesk,sans-serif;color:#f8f5f0;background:#111827;padding:12px;border-radius:8px;min-width:200px;">
            <p style="font-weight:700;font-size:14px;color:#10b981;margin:0 0 6px;">${ticket.ref || '—'}</p>
            ...
            <p style="font-size:11px;font-style:italic;color:#a3c9aa;margin:6px 0 0;">${ticket.summary || ticket.ai_summary || 'No summary'}</p>
          </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold text-[#f8f5f0]">GIS Complaint Map</h2>
          <p className="text-xs text-[#a3c9aa]/50">Live complaint pins across Delhi wards</p>
        </div>
      ...
      <div className="relative rounded-2xl border border-white/10 overflow-hidden" style={{ height: 'calc(100vh - 200px)' }}>
        {!leafletReady && (
          <div className="absolute inset-0 bg-[#0d1117] flex items-center justify-center z-10">
            <div className="text-center">
              ...
              <p className="text-sm text-[#a3c9aa]/50">Loading map...</p>
            </div>
          </div>
        )}
        ...
        {/* Legend */}
        <div className="absolute bottom-4 right-4 z-[1000] bg-[#0d1117]/90 border border-white/10 rounded-xl px-4 py-3 backdrop-blur-sm">
          <p className="text-xs font-semibold text-[#f8f5f0] mb-2">Legend</p>
          {Object.entries(SEV_COLORS).map(([label, color]) => (
            <div key={label} className="flex items-center gap-2 text-xs text-[#a3c9aa]/60 mb-1">
      ...
      <style>{`
        .leaflet-dark-popup .leaflet-popup-content-wrapper { background: #111827; border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; box-shadow: 0 8px 30px rgba(0,0,0,0.5); }
        .leaflet-dark-popup .leaflet-popup-content { margin: 0; }
        .leaflet-dark-popup .leaflet-popup-tip { background: #111827; }
        .leaflet-dark-popup .leaflet-popup-close-button { color: #a3c9aa; }
      `}</style>
    </div>
  );
}
