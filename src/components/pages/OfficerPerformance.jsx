...
const BADGES = ['🥇', '🥈', '🥉'];
const BAR_COLORS = ['#FF9933', '#3b82f6', '#f59e0b', '#8b5cf6', '#ec4899'];
...
export default function OfficerPerformance({ tickets = [] }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-bold text-[white]">Officer Performance</h2>
        <p className="text-xs text-[#8A9BB5]/50">Leaderboard and performance metrics</p>
      </div>

      {/* Leaderboard Table */}
      <div className="rounded-2xl border border-white/10 bg-[#112240]/80 p-5 overflow-x-auto">
              ...
              <tr key={o.name} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                ...
                <td className="py-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#FF9933]/15 flex items-center justify-center text-xs font-bold text-[#FF9933]">
                      {o.name.split(' ').map(n => n[0]).join('')}
                    </div>
                    <div>
                      <p className="text-[white] font-medium">{o.name}</p>
                      <p className="text-[10px] text-[#8A9BB5]/40">{o.dept} Dept</p>
                    </div>
                  </div>
                </td>
                <td className="py-3 text-center text-[#FF9933] font-bold">{o.resolved}</td>
                <td className="py-3 text-center text-[#8A9BB5]/60">{o.avgHours}h</td>
                ...
                <td className="py-3 text-center font-bold text-[white]">{o.score}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData}>
              ...
              <XAxis dataKey="name" tick={{ fontSize: 11, fill: '#8A9BB580' }} axisLine={false} />
