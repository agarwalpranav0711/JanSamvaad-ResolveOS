    ...
    <div className="min-h-screen" style={{ background: '#080c10', fontFamily: "'Space Grotesk', sans-serif" }}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-20">
        <div className="text-center mb-16">
          <span className="px-4 py-1.5 rounded-full bg-[#10b981]/10 border border-[#10b981]/20 text-[#10b981] text-xs font-semibold uppercase tracking-widest">Regulatory Compliance</span>
          <h1 className="text-4xl md:text-5xl font-bold text-[#f8f5f0] mt-6">Fully Compliant</h1>
          <p className="text-lg text-[#a3c9aa]/60 mt-4">Built from day one to meet every Indian telecom and data privacy regulation.</p>
        </div>

        <div className="space-y-8">
          {CHECKLIST.map(section => (
            <div key={section.title} className="rounded-2xl border border-white/10 bg-[#0d1117] p-6">
              <h3 className="text-sm font-bold text-[#f8f5f0] mb-4">{section.title}</h3>
              <div className="space-y-3">
                {section.items.map(item => (
                  <div key={item} className="flex items-start gap-3">
                    <span className="w-5 h-5 rounded-full bg-[#10b981]/20 flex items-center justify-center text-[#10b981] text-xs flex-shrink-0 mt-0.5">✓</span>
                    <span className="text-sm text-[#a3c9aa]/70">{item}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="mt-12 text-center">
          <div className="inline-flex items-center gap-3 px-8 py-4 rounded-2xl border border-[#10b981]/30 bg-[#10b981]/5">
            <span className="text-3xl">🛡️</span>
            <div className="text-left">
              <p className="text-sm font-bold text-[#10b981]">Fully Compliant</p>
              <p className="text-xs text-[#a3c9aa]/50">Indian Telecom Regulations • Data Privacy Act • RTI Act 2005</p>
            </div>
          </div>
        </div>

        {/* IVR Flow Diagram */}
        <div className="mt-16 rounded-2xl border border-white/10 bg-[#0d1117] p-8">
          <h3 className="text-lg font-bold text-[#f8f5f0] mb-6 text-center">Consent IVR Flow</h3>
          <div className="flex flex-col items-center space-y-4">
            {[
              ...
              { step: '🤖 AI Classification + Severity Assignment', color: '#10b981' },
              { step: '🎫 Reference Number Issued + SMS Sent', color: '#10b981' },
            ].map((item, i) => (
              <div key={i} className="w-full max-w-md">
                <div className="flex items-center gap-3 px-5 py-3 rounded-xl border border-white/5 bg-white/[0.02]">
                  ...
                  <span className="text-sm text-[#f8f5f0]">{item.step}</span>
                </div>
